...
- **Scattered items in basket (no grouping)**: 50-item baskets were hard to scan. `BasketList` now groups items by `ShoppingProduct.category` using the canonical `CATEGORY_ORDER` exported from `ProductCatalogArea`, with a sticky per-group header that shows the category icon (`getCategoryIconSrc`) + name + count pill. Products without a category fall into "אחר ולא מסווג". No API or data-model changes — the `category` field was already populated.
- **Non-deliverable chains showing in the comparison strip**: After the user-settings refactor, `StoresStripV2` / `KPIHero` / mobile bar were showing chains that don't deliver to the user's city (e.g. "מחסני השוק" in קריית אונו). `ShoppingInputArea` already computed `effectiveChains` from `deliveryCheck.chains` and piped it into the catalog + smart list, but the live comparison never applied that filter — the backend's `eligible_store_ref_ids` filter wasn't catching every case. Fixed inside `useLiveComparison` so `chains`, `cheapest`, `savingsVsNext` are all delivery-filtered at the hook level (online mode only; physical mode unchanged). The deliverable set stores BOTH the raw delivery-API name (English, e.g. "Market Warehouses") and its `SUPERMARKET_NAME_MAP` translation (Hebrew, e.g. "מחסני השוק") since `ChainTotal.displayName` is Hebrew — matching on either form keeps the filter robust to which side changes. Every downstream component inherits the fix without prop changes.
- **Stale `deliveryCheck` after profile city change**: Saving a new city in `ProfileModal` updated `shoppingCity` but never re-ran `checkDelivery()`, so the chain strip kept filtering against the old city's deliverable set (e.g. changing from קריית אונו to באר שבע, "מחסני השוק" still didn't appear). Added a central `useEffect` in `App.tsx` that watches `shoppingCity` + `shoppingLocation?.streetName` (and `appMode === 'shopping'`) and re-fetches the delivery check. `checkDelivery`'s 10-min LRU de-dupes any overlap with the existing setup/hydration callsites.
- **ProfileModal missing address autocomplete**: When `ProfileModal` was introduced it shipped with plain-text city/street/house fields — no call into the `data.gov.il` streets dataset that `ShoppingSetupStep` has been using via `useAddressAutocomplete`. Wired the same hook into `ProfileModal`: one search input (street + city) with a dropdown, plus a separate free-text "house / apt / entrance" line. Picking a suggestion sets `draftCity`, `draftStreetName`, and `draftCityCode` together so `cityCode` stays consistent with the canonical gov.il city spelling (important because `CITY_NAME_NORMALIZATIONS` in `govDataService.ts` remaps e.g. "קרית אונו" → "קריית אונו" to match the delivery DB).
- **Saved list was overriding profile city/mode on load**: The Firestore sync effect in `App.tsx` (~line 195) used to call `setShoppingCity/setShoppingLocation/setSelectedShoppingMode` from `ListDocument.shoppingCity/shoppingLocation/shoppingMode`. So a user who built a list in City A, then changed their profile to City B, would see City A come back the next time they loaded that list — silently desyncing the comparison from their current profile. Decoupled: the sync effect no longer reads those fields, and the `build_list` vs `setup` decision now keys off the live profile (`shoppingCity && selectedShoppingMode`). Lists still carry a created-at snapshot (see `createShoppingList` in `firestoreService.ts`) as read-only history, but the profile is the single source of truth. Sharing lists is a physical-mode (in-store) feature where delivery address is irrelevant, so no UX penalty.
- **Basket +/- always stepped by 1 regardless of product type**: Catalog cards already step weighted produce by 0.5 kg and deli/dairy by 0.1 kg (see `ProductCard.tsx` / `ProductDetailModal.tsx`), but the basket sidebar hardcoded `amount ± 1` so decrementing a 1.5 kg tomato jumped to 0.5 instead of 1.0 and removing at ≤ 1 dropped the product prematurely. Extracted `getQuantityStep(product)` + `roundQuantity(amount, step)` into `utils/priceFormat.ts` (shared helpers future callers should use; the two existing catalog callers still inline the same rule — migrating them is optional cleanup). `BasketList` now uses these: increments/decrements by the per-product step, floors at one full step (below that removes), and `toFixed`-rounds to guard float drift.
- **"חסרים N" badge hid real missing items on top-tier chains**: `StoresStripV2` computed `missing = maxMatched - c.matchedItems` (relative to the leading chain). So when Rami Levy, Victory, and Shufersal were all short by the same weighted deli item, every chain looked like "0 missing" vs. the leader — yet clicking through to the PricePilot breakdown showed "4/5 מוצרים". Changed the math to be absolute: `missing = totalItems - c.matchedItems`. Added `totalItems` (cart size at comparison time) to `LiveComparisonResult`, threaded through `ShoppingInputArea` → `StoresStripV2`. Also hardened `ChainTotal.matchedItems` in the hook to use `StorePriceSummary.itemPrices.length` (the priced subset that PricePilot actually builds from) rather than the raw API `matched_items`, so the strip and the breakdown stay consistent even if the backend ever drifts on that flag.
