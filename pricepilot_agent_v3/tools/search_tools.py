...
async def search_products(query: str, store_id: int = 0, max_results: int = 5) -> dict:
    ...
    # search_products has no tool_context, so we use a fixed session key.
    # Tools that DO have tool_context pass the real session_id.
    _sid = "search"
    observer.log_tool_start(_sid, "search_products", {
        "query": query, "store_id": store_id, "max_results": max_results,
    })
    ...
    for item in items[:max_results]:
        ...
        product = {
            "product_id": item.get("id", ""),  # INTERNAL Rami Levy ID — use this for add_items_to_cart
            "barcode": str(item.get("barcode", "")),
            "name": item.get("name", ""),
            "price": item.get("price1") or item.get("price2") or item.get("price", 0),
            "in_stock": item.get("in_stock", True),
            "is_weighted": is_weighted,  # True = sold by kg, False = sold per unit
            "multiplication": multiplication,  # Default weight step in kg (e.g. 0.5 = each qty unit adds 0.5kg)
            "unit_info": f"{'per kg, step=' + str(multiplication) + 'kg' if is_weighted else 'per unit'}",
        }
        # Include store-specific price fields if present
    ...
    if not products:
        ...
        observer.log_tool_end(_sid, "search_products", result)
        return result
    ...
    observer.log_tool_end(_sid, "search_products", {"count": len(products), "status": "success"})
    return result
